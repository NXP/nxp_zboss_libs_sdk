...
static zb_cluster_def cluster_0003 = {
  ...
  identify_commands_handler,
};


/* -------------------------------- Cli commands Identify --------------------------- */


/* Static stack callback function
 * response for identify_cmd
 */
static void identify_cmd_cb(zb_uint8_t param)
{
  zb_zcl_command_send_status_t *cmd_send_status = ZB_BUF_GET_PARAM(param, zb_zcl_command_send_status_t);

  menu_printf("identify_cmd_cb(0x%04x) %s", cmd_send_status->dst_addr.u.short_addr, wcs_get_error_str(cmd_send_status->status));
...
static zb_ret_t cluster_identify_cmd(enum zb_zcl_identify_cmd_e new_command, int argc, char *argv[]);
...
static zb_ret_t help_ident_cmds(void);
#endif
static zb_ret_t help_ident_cmds_detailed(char *subcommand);
static zb_ret_t cluster_ident_submenu(int argc, char *argv[])
{
  zb_ret_t ret;
  zb_uint8_t command_id;
  cli_tools_strval cmd_table[] = {
    { "identify",                ZB_ZCL_CMD_IDENTIFY_IDENTIFY_ID },
    { "identify_query",          ZB_ZCL_CMD_IDENTIFY_IDENTIFY_QUERY_ID }
  };
  ...
  if(tools_arg_help(argc, argv, 1) == RET_OK)
    return help_ident_cmds_detailed(argv[0]);
  ...
  if(ret == RET_OK)
  {
    switch(command_id)
    {
      case ZB_ZCL_CMD_IDENTIFY_IDENTIFY_ID:
        return cluster_identify_cmd(command_id, argc-1, &argv[1]);
  ...
  menu_printf("cluster identify %s: unknown command", argv[0]);
#ifdef MENU_PRINT_HELP_IF_COMMAND_NOT_FOUND
  menu_printf("");
  help_ident_cmds();
#endif
...
static zb_ret_t help_ident_cmds(void) { return help_ident_cmds_detailed(NULL); }
static zb_ret_t help_ident_cmds_detailed(char *subcommand)
{
  /* SUBMENU identify Commands */
  if(!subcommand || !strcmp(subcommand, "identify"))
  {
    menu_printf("cluster identify_cmd identify [endpoint] [dest_addr] [dest_ep] [time]:");
    menu_printf("\tsend ZCL Command identify from endpoint [0-255] to a specific device at dest_addr [0xAAAA] endpoint [0-255] and set identification time [0-65535]");
  }
  if(!subcommand || !strcmp(subcommand, "identify_query"))
  {
    menu_printf("cluster identify_cmd identify_query [endpoint] [dest_addr] [dest_ep]:");
    menu_printf("\tsend ZCL Command identify_query from endpoint [0-255] to a specific device at dest_addr [0xAAAA] endpoint [0-255]");
  }
  return RET_OK;
}
...
static zb_ret_t cluster_identify_cmd(enum zb_zcl_identify_cmd_e new_command, int argc, char *argv[])
{
  ...
  /*  get [endpoint] & validate it has cluster identify client */
  TOOLS_GET_ARG_ENDPOINT_W_CL(ret, argv, 0, &this_ep, ZB_ZCL_CLUSTER_ID_IDENTIFY, ZB_ZCL_CLUSTER_CLIENT_ROLE);
  ...
  if(argc == 4)
  {
    TOOLS_GET_ARG(ret, uint16,  argv, 3, &time);
  }
  ...
  /* Do it */
  {
    ...
    switch(new_command) {
    case ZB_ZCL_CMD_IDENTIFY_IDENTIFY_ID:
      /*! @brief General macro for sending Identify command
          @params buffer, time, addr, dst_addr_mode, dst_ep, ep, prof_id, dis_default_resp, command_id, cb */
      ZB_ZCL_IDENTIFY_SEND_IDENTIFY_REQ(buffer, time, dest_short_addr, ZB_APS_ADDR_MODE_16_ENDP_PRESENT, dest_ep_id, this_ep->ep_id, this_ep->profile_id, ZB_FALSE, identify_cmd_cb);
      break;

    case ZB_ZCL_CMD_IDENTIFY_IDENTIFY_QUERY_ID:
      /*! @brief General macro for sending Identify Query command
          @params buffer, addr, dst_addr_mode, dst_ep, ep, prof_id, dis_default_resp, command_id, cb */
      ZB_ZCL_IDENTIFY_SEND_IDENTIFY_QUERY_REQ(buffer, dest_short_addr, ZB_APS_ADDR_MODE_16_ENDP_PRESENT, dest_ep_id, this_ep->ep_id, this_ep->profile_id, ZB_TRUE, identify_cmd_cb);
      break;
...
/* -------------------------------- Handler commands Identify --------------------------- */


static zb_uint8_t identify_server_commands_handler(zb_zcl_parsed_hdr_t *cmd_info, zb_uint8_t param)
{
  ...
  switch(cmd_info->cmd_id) {
    case ZB_ZCL_CMD_IDENTIFY_IDENTIFY_ID:
      {
        zb_zcl_identify_req_t identify_req = {0};

        ZB_ZCL_IDENTIFY_GET_IDENTIFY_REQ(&identify_req, param, status);

        if(status == ZB_ZCL_PARSE_STATUS_SUCCESS)
          menu_printf("%s() recv from 0x%04x endpoint %u: IDENTIFY timeout %u",
            __FUNCTION__,
            ZB_ZCL_PARSED_HDR_SHORT_DATA(cmd_info).source.u.short_addr,
            ZB_ZCL_PARSED_HDR_SHORT_DATA(cmd_info).src_endpoint,
            identify_req.timeout);
       else
          menu_printf( "\tERROR: cannot extract identify_req");
      }
      break;
    case ZB_ZCL_CMD_IDENTIFY_IDENTIFY_QUERY_ID:
      {
        menu_printf("%s() recv from 0x%04x endpoint %u: IDENTIFY_QUERY",
          __FUNCTION__,
          ZB_ZCL_PARSED_HDR_SHORT_DATA(cmd_info).source.u.short_addr,
          ZB_ZCL_PARSED_HDR_SHORT_DATA(cmd_info).src_endpoint);
      }
...
static zb_uint8_t identify_client_commands_handler(zb_zcl_parsed_hdr_t *cmd_info, zb_uint8_t param)
{
  zb_uint8_t ret = ZB_FALSE;
  zb_zcl_parse_status_t status;

  switch(cmd_info->cmd_id) {
    case ZB_ZCL_CMD_IDENTIFY_IDENTIFY_QUERY_RSP_ID:
      {
        zb_zcl_identify_query_res_t identify_query_resp;

        ZB_ZCL_IDENTIFY_GET_IDENTIFY_QUERY_RES(&identify_query_resp, param, status);

        if(status == ZB_ZCL_PARSE_STATUS_SUCCESS)
          menu_printf("%s() recv from 0x%04x endpoint %u: IDENTIFY_QUERY_RSP timeout %d",
            __FUNCTION__,
            ZB_ZCL_PARSED_HDR_SHORT_DATA(cmd_info).source.u.short_addr,
            ZB_ZCL_PARSED_HDR_SHORT_DATA(cmd_info).src_endpoint,
            identify_query_resp.timeout);
        else
          menu_printf( "\tERROR: cannot extract identify_query_resp");
      }
      break;
...
//          case : my_cluster_attr_len = sizeof()); break;
          ...
          /* Store this efs data */
          if(!this_efs->data)
            this_efs->data       = ZB_MALLOC(fieldset->fieldset_length);
          else
            this_efs->data       = ZB_REALLOC(this_efs->data, fieldset->fieldset_length);
          if(!this_efs->data)
          {
          ...
          ZB_MEMCPY(this_efs->data, fs_data_ptr, fieldset->fieldset_length);
      ...
      ZB_MEMCPY(&this_scene_db->info, add_scene_req, sizeof(this_scene_db->info));
      ...
      if(scene_found)
      {
        ...
        *remove_scene_status = ZB_ZCL_STATUS_SUCCESS;
        ...
        for(int i = 0; i < this_scene_db->ext_field_set_cnt; i++)
        {
          ...
          ZB_FREE(this_efs->data);
        }
        ZB_MEMSET(this_scene_db, 0, sizeof(*this_scene_db));
      }
