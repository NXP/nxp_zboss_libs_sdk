...
/* Flag that shows we're currently in rejoin context */
static zb_bool_t is_rejoin = ZB_FALSE;
...
#else
zb_zcl_attr_t ota_upgrade_attr_list[] = { { ZB_ZCL_NULL_ID, 0, 0, ZB_ZCL_NON_MANUFACTURER_SPECIFIC, NULL } };
#endif
...
#else
zb_zcl_attr_t wwah_attr_list[] = { { ZB_ZCL_NULL_ID, 0, 0, ZB_ZCL_NON_MANUFACTURER_SPECIFIC, NULL } };
#endif
...
#ifdef IZS_DISTRIBUTED_ENABLE
  zb_enable_joining_to_distributed_network();
#endif

  zb_set_long_address(ieee_addr);

  ZB_ZCL_SET_STRING_VAL(g_device_ctx.basic_attr.mf_name, IZS_INIT_BASIC_MANUF_NAME, ZB_ZCL_STRING_CONST_SIZE(IZS_INIT_BASIC_MANUF_NAME));
  ZB_ZCL_SET_STRING_VAL(g_device_ctx.basic_attr.model_id, IZS_INIT_BASIC_DEFAULT_MODEL_ID, ZB_ZCL_STRING_CONST_SIZE(IZS_INIT_BASIC_DEFAULT_MODEL_ID));
}
...
void izs_clusters_attr_init(zb_uint8_t param)
{
  ...
  /* Needed for WWAH Tests compliance*/
...
void izs_leave_indication(zb_uint8_t leave_type)
{
  ...
  /* Rejoin already started in stack */
  else if (leave_type == ZB_NWK_LEAVE_TYPE_REJOIN)
  {
    is_rejoin = ZB_TRUE;
  }
...
void izs_find_ias_zone_client_cb(zb_uint8_t buf)
{
  ...
  TRACE_MSG(TRACE_APP1, ">> izs_find_ias_zone_client %hd", (FMT__H, buf));
  ...
  TRACE_MSG(TRACE_APP1, "<< izs_find_ias_zone_client", (FMT__0));
}
...
#ifdef ZB_SAMPLES_USE_REJOIN_BACKOFF
#if defined IZS_WWAH && defined ZB_USE_SLEEP
        if (!zb_zcl_wwah_check_if_wwah_rejoin_enabled())
#endif
        {
          /* Successfully (re)joined. Clear rejoin backoff context (even it was not used...) */
          zb_zdo_rejoin_backoff_cancel();
        }
#endif /* ZB_SAMPLES_USE_REJOIN_BACKOFF */
        is_rejoin = ZB_FALSE;
...
//! [signal_sleep]
  ...
  else
  {
    ...
    if (IZS_DEVICE_IS_ENROLLED() || is_rejoin)
    {
#ifdef ZB_SAMPLES_USE_REJOIN_BACKOFF
...
void izs_retry_join(void)
{
  /*
    ...
    enrolled and then lost network, rejoin backoff algorithm starts rejoin_backoff
  */
...
#ifdef ZB_SAMPLES_USE_REJOIN_BACKOFF
  else if (g_device_ctx.retry_backoff_cnt || is_rejoin)
  {
    zb_ret_t status;
    /* We are in "retry" scenario - do not call command send */
    /* But force retry timeout to expire instead */
...
zb_uint16_t izs_wwah_app_event_retry_get_next_timeout(zb_uint8_t failed_attempts)
{
  ...
  for (i = 0; i + 1 < failed_attempts && timeout < MAX_BACKOFF_TIME(); i++)
  {
