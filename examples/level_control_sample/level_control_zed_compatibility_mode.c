...
#define DEFAULT_PEER_REVISION 1
...
MAIN()
{
  ...
  ZB_INIT("level_control_zed_compatibility_mode");
...
void add_peer_revision_entry(zb_ieee_addr_t ieee_addr, zb_uint16_t cluster_id, zb_uint8_t cluster_role, zb_uint8_t endpoint, zb_uint16_t peer_revision)
{
  TRACE_MSG(TRACE_APP1, "Add peer revision entry", (FMT__0));
  ZB_IEEE_ADDR_COPY(g_peer_table[0].ieee_addr, ieee_addr);
  g_peer_table[0].cluster_id = cluster_id;
  g_peer_table[0].cluster_role = cluster_role;
  g_peer_table[0].endpoint = endpoint;
  g_peer_table[0].peer_revision = peer_revision;
}

static void read_attr_resp_handler(zb_bufid_t cmd_buf)
{
  zb_zcl_read_attr_res_t * read_attr_resp;
  zb_zcl_parsed_hdr_t *cmd_info = ZB_BUF_GET_PARAM(cmd_buf, zb_zcl_parsed_hdr_t);
  ...
  if (read_attr_resp)
  {
    ...
    {
      if (ZB_ZCL_ATTR_GLOBAL_CLUSTER_REVISION_ID == read_attr_resp->attr_id)
      {
        zb_ieee_addr_t ieee_addr;
        zb_bufid_t buf = zb_buf_get_out();
        zb_address_ieee_by_short(cmd_info->addr_data.common_data.source.u.short_addr, ieee_addr);
        add_peer_revision_entry(ieee_addr, cmd_info->cluster_id,
                                (ZB_ZCL_FRAME_DIRECTION_TO_CLI == cmd_info->cmd_direction) ? (ZB_ZCL_CLUSTER_SERVER_ROLE) : (ZB_ZCL_CLUSTER_CLIENT_ROLE),
                                cmd_info->addr_data.common_data.src_endpoint, ZB_ZCL_ATTR_GET16(read_attr_resp->attr_value));
        TRACE_MSG(TRACE_APP3, "Peer revision: %d", (FMT__D, ZB_ZCL_ATTR_GET16(read_attr_resp->attr_value)));
...
{
  TRACE_MSG(TRACE_APP1, "peer revision callback callback", (FMT__0));
  if (ZB_IEEE_ADDR_CMP(ieee_addr, g_peer_table[0].ieee_addr)
      && g_peer_table[0].endpoint == endpoint
      && g_peer_table[0].cluster_id == cluster_id
      && g_peer_table[0].cluster_role == cluster_role)
  {
    return g_peer_table[0].peer_revision;
  }
  else
  {
    return DEFAULT_PEER_REVISION;
  }
}
...
void send_read_attr_cluster_revision(zb_uint8_t param)
{
  zb_uint8_t *cmd_ptr;
  ...
  ZB_ZCL_GENERAL_SEND_READ_ATTR_REQ(
      (param), cmd_ptr, DST_ADDR, ZB_APS_ADDR_MODE_16_ENDP_PRESENT, ZB_LEVEL_CONTROL_SERVER_ENDPOINT, ZB_LEVEL_CONTROL_CLIENT_ENDPOINT,
       ZB_AF_HA_PROFILE_ID, ZB_ZCL_CLUSTER_ID_LEVEL_CONTROL, NULL);
}
...
/* [zb_bdb_finding_binding_initiator] */
      ...
      case ZB_BDB_SIGNAL_FINDING_AND_BINDING_INITIATOR_FINISHED:
      {
        ...
        /* There is no necessity to call zb_zcl_set_backward_comp_mode(ZB_ZCL_COMPATIBILITY_MODE) directly.
         * It is a part of zb_zcl_set_peer_revision_callback
         */
        zb_zcl_set_peer_revision_callback(peer_revision_cb);
        ZB_SCHEDULE_APP_CALLBACK(send_read_attr_cluster_revision, buf);
      }
      break;
