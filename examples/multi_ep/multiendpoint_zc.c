...
MAIN()
{
  ...
  zb_nwk_set_max_ed_capacity(MULTI_EP_ZC_DEV_NUMBER);
...
/* Write/Read Steps */
    ...
    case 1:
      ...
      TRACE_MSG(TRACE_APP1, "READ_ATTR: src_ep=%hd, dst_ep=%hd, cluster=ZB_ZCL_CLUSTER_ID_ON_OFF" \
                ", attr=ZB_ZCL_ATTR_ON_OFF_ON_OFF_ID", (FMT__H_H, ZC_EP1, ZED_EP1));
...
/* Command steps */
    ...
    case 16:
      /* Send OFF from ZC_EP4 to ZED_EP4 (EP4 does not support ON_OFF cluster, default response must report "unsupported cluster") */
    ...
    case 17:
      ...
      break;
    case 18:
      /* Send OFF to an EP that does not support ON_OFF cluster (since this is sent unicast, default response must report "Unsupported Cluster")*/
                              ...
                              zb_uint16_t attr_id, zb_uint8_t attr_type, zb_uint8_t *value)
{
  ...
  TRACE_MSG(TRACE_APP1, ">> ATTR_REPORT: addr=%d, ep=%hd, cluster=0x%x, attr=%d, value=%d, attr_type=%x",
            (FMT__D_H_D_D_D_D, addr->u.short_addr, ep, cluster_id, attr_id, *value, attr_type));
...
static void simple_desc_callback(zb_bufid_t param)
{
  ...
  TRACE_MSG(TRACE_APP1, ">> simple_desc_callback: status %hd, addr 0x%x",
            (FMT__H_H, resp->hdr.status, resp->hdr.nwk_addr));
...
void active_ep_callback(zb_bufid_t param)
{
  ...
  if (dev_idx != MULTI_EP_ZC_INVALID_DEV_INDEX)
  {
    ...
    {
        TRACE_MSG(TRACE_APP1, "active_ep_resp: Error incorrect status", (FMT__0));
    } else
    {
      ...
      {
        TRACE_MSG(TRACE_APP1, "No memory to store all EndPoints", (FMT__0));
      } else
      {
        ...
        for (int i = 0; i < resp->ep_count; i++)
        {
          ...
          TRACE_MSG(TRACE_APP1, "active_ep_resp: ep %hd", (FMT__H, *(ep_list + i)));
