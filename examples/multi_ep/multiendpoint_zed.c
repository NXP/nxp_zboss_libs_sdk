...
void configure_local_binding(zb_bufid_t param)
{
  ...
  else
  {
    ...
    ZB_IEEE_ADDR_COPY(req->dst_address.addr_long, coordinator_addr);

    /* Bind On/Off cluster 1 */
    req->src_endp = MULTI_EP_ZED_ENDPOINT_EP1;
    ...
    zb_zdo_bind_req(param, configure_local_binding_completed);
