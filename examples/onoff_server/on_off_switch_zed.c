...
/* If defined, the network is closed after ZED successfully joined  */
#undef CLOSE_NWK_AFTER_JOIN
...
/* automatically send toggle_req until a button is pressed */
...
zb_uint8_t zcl_specific_cluster_cmd_handler(zb_uint8_t param)
{
  ...
  WCS_TRACE_DEBUG("cmd_handler() seq_num %d, %s, %s, cluster %s, profile %04x",
      cmd_info->seq_number,
...
void send_toggle_req_and_schedule_alarm(zb_uint8_t param)
{
  if (!param)
  {
    ...
    zb_buf_get_out_delayed(send_toggle_req_and_schedule_alarm);
  }
  else
  {
