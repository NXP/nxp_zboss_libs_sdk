...
/* In current implementation, simple gw is a concentrator */
#define ZB_EXAMPLE_CONCENTRATOR_MODE_ENABLE
...
#ifdef ZB_EXAMPLE_CONCENTRATOR_MODE_ENABLE
static void start_concentrator_mode(zb_uint8_t radius, zb_uint32_t disc_time);
#endif /* ZB_EXAMPLE_CONCENTRATOR_MODE_ENABLE */
...
MAIN()
{
  ...
  zb_nwk_set_max_ed_capacity(SIMPLE_GW_DEV_NUMBER);
...
static void start_concentrator_mode(zb_uint8_t radius, zb_uint32_t disc_time)
{
  ...
  if (is_already_started == ZB_FALSE){
    ...
    WCS_TRACE_DBGREL("concentrator config: radius %hd, disc_time %lu", radius, disc_time);
  ...
  {
    WCS_TRACE_DEBUG("concentrator mode already started on previous dev announce reception");
    TRACE_MSG(TRACE_TRANSPORT3, "concentrator mode already started on previous dev announce reception", (FMT__0));
  }
  TRACE_MSG(TRACE_TRANSPORT3, "<< start_concentrator_mode", (FMT__0));
}
#endif /* ZB_EXAMPLE_CONCENTRATOR_MODE_ENABLE */
...
/* [signal_device_annce] */
      ...
      {
#ifdef ZB_EXAMPLE_CONCENTRATOR_MODE_ENABLE
        zb_uint8_t radius = 15; /* Default value */
        zb_int32_t env_disc_time = 0;/* Default value is 0: No periodical route discoveries */
        char *tmp = getenv("ZB_CONCENTRATOR_DISC_TIME");
#endif
...
#ifdef ZB_EXAMPLE_CONCENTRATOR_MODE_ENABLE
        if (tmp && 1 == sscanf(tmp, "%d", &env_disc_time))
        {
          if(env_disc_time >= 0)
          {
            start_concentrator_mode(radius, env_disc_time);
          }
          else
          {
            WCS_TRACE_DBGREL("ZB_CONCENTRATOR_DISC_TIME error: must be a positive value");
          }
        }
#endif
