...
void permit_join_request_broadcast(zb_uint8_t buf, zb_uint16_t time);
void permit_join_request_broadcast_cb(zb_uint8_t param);

void permit_join_request_local(zb_uint8_t buf, zb_uint16_t time);
void permit_join_request_local_cb(zb_uint8_t param);
...
void send_get_scene_membership_resp(zb_uint8_t param)
{
  ...
  zb_bool_t is_unicast = 
    (zb_bool_t) (ZB_APS_FC_GET_DELIVERY_MODE(resp_info.cmd_info.addr_data.common_data.fc) == ZB_APS_DELIVERY_UNICAST);

  TRACE_MSG(TRACE_APP1, ">> send_get_scene_membership_resp param %hd", (FMT__H, param));

  if (resp_info.get_scene_membership_req.group_id > ZB_ZCL_ATTR_SCENES_CURRENT_GROUP_MAX_VALUE)
  {
    /* Not in the group */
  ...
  /* ZCL8 Section 3.7.2.4.8.2 Item 3:
     ...
     SHALL transmit it back to the originator of the Get Scene Membership command.
     If the Get Scene Membership command was not received as a unicast, the device SHALL only generate
     a Get Scene Membership Response command with the Status field set to the evaluated status if an
     entry within the Scene Table corresponds to the Group ID; the device SHALL then transmit it back
     to the originator of the Get Scene Membership command */
  if (is_unicast ||
      (!is_unicast && ZB_ZCL_DEVICE_CMD_PARAM_STATUS(param) == ZB_ZCL_STATUS_SUCCESS))
  {
...
/* [permit_join_request_broadcast_example] */
void permit_join_request_broadcast(zb_uint8_t buf, zb_uint16_t time)
{
  TRACE_MSG(TRACE_APP3, ">> permit_join_request_broadcast : param %hd", (FMT__D, buf));
  if (buf == 0U)
  {
    zb_buf_get_out_delayed_ext(permit_join_request_broadcast, time, 0U);
  }
  else
  {
    ...
    TRACE_MSG(TRACE_APP1, "permit_join_request_broadcast_cb: requesting permit join", (FMT__0));

    req_param->dest_addr = ZB_NWK_BROADCAST_ROUTER_COORDINATOR;
    req_param->permit_duration = (zb_uint8_t) time;
    req_param->tc_significance = ZB_TRUE;

    zb_zdo_mgmt_permit_joining_req(buf, &permit_join_request_broadcast_cb);
  }
}
...
void permit_join_request_broadcast_cb(zb_uint8_t param)
{
  ...
  TRACE_MSG(TRACE_APP1, "permit_join_request_broadcast_confirm_cb: callback called", (FMT__0));
  return;
}
/* [permit_join_request_broadcast_example] */

/* [permit_join_request_local_example] */
void permit_join_request_local(zb_uint8_t buf, zb_uint16_t time)
{
  TRACE_MSG(TRACE_APP3, ">> permit_join_request_local : param %hd", (FMT__D, buf));

  if (buf == 0U)
  {
    zb_buf_get_out_delayed_ext(permit_join_request_local, time, 0U);
  }
  else
  {
    zb_zdo_mgmt_permit_joining_req_param_t *req_param
    ...
    TRACE_MSG(TRACE_APP1, "permit_join_request_local_cb: requesting permit join", (FMT__0));

    req_param->dest_addr = zb_get_short_address();
    req_param->permit_duration = (zb_uint8_t) time;
    req_param->tc_significance = ZB_TRUE;

    zb_zdo_mgmt_permit_joining_req(buf, &permit_join_request_local_cb);
  }
}
...
void permit_join_request_local_cb(zb_uint8_t param)
{
  ...
  TRACE_MSG(TRACE_APP1, "permit_join_request_local_cb: callback called", (FMT__0));
  return;
}
/* [permit_join_request_local_example] */

/* [close_network_example] */
static void close_network(zb_uint8_t param)
{
  TRACE_MSG(TRACE_APP1, ">> close network", (FMT__0));
  zb_bdb_close_network(param);
}
/* [close_network_example] */

static void on_device_started(void)
{
  ...
  if (ZB_GET_APP_SIGNAL_STATUS(param) == 0)
  {
    switch(sig)
    {
      ...
      case ZB_BDB_SIGNAL_DEVICE_FIRST_START:
        ...
        ZB_SCHEDULE_APP_CALLBACK2(permit_join_request_broadcast, 0U, 234U);
        ZB_SCHEDULE_APP_CALLBACK2(permit_join_request_local, 0U, 123U);

        ZB_SCHEDULE_APP_ALARM(close_network, 0U, 10U * ZB_TIME_ONE_SECOND);
        break;
      ...
      /* [zb_zdo_signal_permit_join_status_example] */
