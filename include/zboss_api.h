...
/**
 * @brief A struct with params for zb_get_pan_channel_async
 */
typedef struct zb_get_pan_channel_params_s
{
  zb_ret_t status;                /*!<  Status of the operation. Can be RET_OK or RET_UNINITIALIZED or RET_TIMEOUT */
  zb_uint8_t page;                /*!< Page number. Should be provided by the application. */
  zb_uint8_t current_pan_channel; /*!< 15.4 operating PAN Channel number on a given page, for this current PAN and has been given by this application. */
  zb_uint8_t side_pan_channel;    /*!< 15.4 PAN Channel number for the other PAN. This is provided by the other PAN application in case of DualPAN. */
  zb_callback_t cb;               /*!< Callback function to be called after the function finishes. Should be provided by the application. */
} zb_get_pan_channel_params_t;

/**
 * @brief Get transceiver side PAN channel configured by Thread asynchronously.
 *
 * This function requires param to contain @ref zb_get_pan_channel_params_t.
 * Will return status RET_UNINITIALIZED/0 if the channel hasn't been initialized yet.
 *
 * @param param - buffer, containing @ref zb_get_pan_channel_params_t.
*/
void zb_get_pan_channel_async(zb_bufid_t param);

/**
 * @brief A struct with params for zb_set_pan_channel_async
 */
typedef struct zb_set_pan_channel_params_s
{
  zb_ret_t status;                /*!< Status of the operation. Can be RET_OK, RET_UNINITIALIZED or RET_TIMEOUT or one of
                                       RET_INVALID_PARAMETER_1 or RET_INVALID_PARAMETER_2
                                       for the following three fields respectively. */
  zb_uint8_t page;                /*!< Page number. Should be provided by the application. */
  zb_uint8_t current_pan_channel; /*!< 15.4 operating PAN Channel number on a given page. Should be provided by the application. */
  zb_callback_t cb;               /*!< Callback function to be called after the function finishes. Should be provided by the application. */
} zb_set_pan_channel_params_t;

/**
 * @brief Set transceiver side PAN channel asynchronously.
 *
 * This function requires param to contain @ref zb_set_pan_channel_params_t.
 * Will return status RET_INVALID_PARAMETER if the given parameter cannot be configured.
 *
 * @param param - buffer, containing @ref zb_set_pan_channel_params_t.
*/
void zb_set_pan_channel_async(zb_bufid_t param);

#if defined(ZB_MACSPLIT_HOST) && defined(ZB_MAC_INTERFACE_SINGLE)
/** Moved here for the Multi MAC compatibility reasons
 *
 * @warning do not use directly, use @ref zb_get_pan_channel_async instead
 */
void zb_get_pan_channel_macsplit(zb_bufid_t param);

/** Moved here for the Multi MAC compatibility reasons
 *
 * @warning do not use directly, use @ref zb_set_pan_channel_async instead
 */
void zb_set_pan_channel_macsplit(zb_bufid_t param);

/** @endcond */ /* DOXYGEN_BDB_SECTION */

#define zb_get_pan_channel_async zb_get_pan_channel_macsplit
#define zb_set_pan_channel_async zb_set_pan_channel_macsplit
...
/**
 * @brief A struct with params for zb_get_tx_power_capabilities
 */
typedef struct zb_tx_power_capabilities_params_s
{
  zb_ret_t status;    /*!< Status of the operation. */
  zb_int8_t max[16];  /*!< Transceiver maximum power for all channels. */
  zb_int8_t min[16];  /*!< Transceiver minimum power for all channels. */
  zb_callback_t cb;   /*!< Callback function to be called after the function finishes. Should be provided by the application. */
} zb_tx_power_capabilities_params_t;

/**
 * @brief Get transceiver power capabilities (minimum and maximum TX power)
 *
 * This function requires param to contain @ref zb_tx_power_capabilities_params_t.
 *
 * @param param - buffer, containing @ref zb_tx_power_capabilities_params_t.
*/
void zb_get_tx_power_capabilities(zb_bufid_t param);
#endif

#ifdef NXP_GET_RX_SENSITIVITY
/** @brief Response for zb_rx_sensitivity_req
  */
typedef struct zb_rx_sensitivity_params_s
{
  zb_ret_t  status;          /*!< Status of the operation. */
  zb_int8_t rx_sensitivity;  /*!< The status of the command.*/
  zb_callback_t cb;   /*!< Callback function to be called after the function finishes. Should be provided by the application. */
}zb_rx_sensitivity_params_t;

/**
 * @brief Get transceiver rx sensitivity (noise floor)
 *
 * This function requires param to contain @ref zb_rx_sensitivity_params_t.
 *
 * @param param - buffer, containing @ref zb_rx_sensitivity_params_t.
*/
void zb_get_rx_sensitivity(zb_bufid_t param);
#endif
 ...
 *  @param version_id - Version type requested: HOST_VERSION, STACK_VERSION, FIRMWARE_VERSION
 *  @returns Pointer to zero-terminated version string.
 */
const zb_char_t ZB_IAR_CODE *zb_get_version(zb_version_type_t version_id);
...
/**
 * Get Manufacturer code of Node Descriptor Request
 *
 * @return manuf_code - current manufacturer
*/
zb_uint16_t zb_get_node_descriptor_manufacturer_code(void);
