...
static zb_ret_t cluster_create(int argc, char *argv[])
{
  ...
  /* Params are good, insert that new endpoint in our context */
  {
    ...
    ZB_MEMSET(new_cl, 0, sizeof(zb_zcl_cluster_desc_t));
...
static zb_ret_t cluster_write_attr(int argc, char *argv[])
{
  ...
  dest_attr_val = ZB_MALLOC(dest_attr_len);
  if(!dest_attr_val)
...
static void continue_disc_attr_cb(zb_uint16_t start_attr_id);
...
disc_attr_ctx_t g_disc_attr_ctx = {0};

void continue_disc_attr_wrapper(zb_uint8_t param)
{
    ZVUNUSED(param);
    continue_disc_attr_cb(g_disc_attr_ctx.next_attr_id);
}

/* Static stack callback function
 * disc_attr continuation if incomplete
 */
void continue_disc_attr_cb(zb_uint16_t start_attr_id)
{
  zb_bufid_t buffer = ZB_BUF_INVALID;
  zb_uint8_t *cmd_ptr;

  /* Store what we're requesting so we can detect loops later */
  g_disc_attr_ctx.start_attr_id = start_attr_id;

  buffer = zb_buf_get_out();

  cmd_ptr = ZB_ZCL_START_PACKET(buffer);
  if(g_disc_attr_ctx.manuf_code != 0xFFFF) {
    ...
    ZB_ZCL_CONSTRUCT_COMMAND_HEADER_EXT(cmd_ptr, ZB_ZCL_GET_SEQ_NUM(), ZB_ZCL_MANUFACTURER_SPECIFIC, g_disc_attr_ctx.manuf_code, ZB_ZCL_CMD_DISC_ATTRIB);
  } else {
  ...
  ZB_ZCL_PACKET_PUT_DATA16_VAL(cmd_ptr, g_disc_attr_ctx.start_attr_id);
  ZB_ZCL_PACKET_PUT_DATA8(cmd_ptr, (zb_uint8_t)(g_disc_attr_ctx.max_count - g_disc_attr_ctx.total_attrs));
  if(!g_disc_attr_ctx.use_extra_options)
  {
    ZB_ZCL_FINISH_N_SEND_PACKET(buffer, cmd_ptr, g_disc_attr_ctx.dest_addr, ZB_APS_ADDR_MODE_16_ENDP_PRESENT, g_disc_attr_ctx.dest_ep, g_disc_attr_ctx.src_ep,
      g_disc_attr_ctx.profile_id, g_disc_attr_ctx.cluster_id, disc_attr_cb);
  }
  else
  {
    ZB_ZCL_FINISH_N_SEND_PACKET_NEW(buffer, cmd_ptr, g_disc_attr_ctx.dest_addr, ZB_APS_ADDR_MODE_16_ENDP_PRESENT,  g_disc_attr_ctx.dest_ep, g_disc_attr_ctx.src_ep,
      g_disc_attr_ctx.profile_id, g_disc_attr_ctx.cluster_id, disc_attr_cb, g_disc_attr_ctx.aps_secured, g_disc_attr_ctx.disable_aps_ack, g_disc_attr_ctx.delay);
  }
...
static zb_ret_t cluster_disc_attr(int argc, char *argv[])
{
  ...
  /* Save context for automatic continuation */
  g_disc_attr_ctx.dest_addr = dest_short_addr;
  g_disc_attr_ctx.dest_ep = dest_ep_id;
  g_disc_attr_ctx.src_ep = this_ep->ep_id;
  g_disc_attr_ctx.profile_id = this_ep->profile_id;
  g_disc_attr_ctx.cluster_id = dest_cluster->id;
  g_disc_attr_ctx.manuf_code = manuf_code;
  g_disc_attr_ctx.max_count = new_max_number;

  /* Optional */
  g_disc_attr_ctx.use_extra_options = use_extra_options;
  g_disc_attr_ctx.aps_secured = aps_secured;
  g_disc_attr_ctx.disable_aps_ack = disable_aps_ack;
  g_disc_attr_ctx.delay = delay;

  /* Reset total counter if starting from beginning */
  if (new_start_index == 0)
  {
    g_disc_attr_ctx.total_attrs = 0;
  }
...
static zb_ret_t cluster_write_local(int argc, char *argv[])
{
  ...
  my_attr_val = ZB_MALLOC(my_attr_len);
  if(!my_attr_val)
    return RET_NO_MEMORY;
  /* get [val data] */
...
static zb_ret_t cluster_raw_cmd(int argc, char *argv[])
{
  ...
  if(argc == 6)
  {
    ...
    dest_payload_data = ZB_MALLOC(sizeof(zb_uint8_t)*dest_payload_len);
  ...
  if (!ZB_JOINED())
  {
    if(dest_payload_data) ZB_FREE(dest_payload_data);
    return RET_UNAUTHORIZED;
  }
...
static zb_ret_t cluster_zcl_aps_cmd(int argc, char *argv[])
{
  ...
  /* Do it */
  {
    ...
    cmd_ptr = ZB_MALLOC(cmd_header_size + dest_payload_len);
    if(!cmd_ptr)
      return RET_NO_MEMORY;
    ...
    ZB_MEMCPY(cmd_ptr, dest_payload_data, dest_payload_len);
    ...
    if(cmd_ptr) ZB_FREE(cmd_ptr-cmd_header_size);
  }
