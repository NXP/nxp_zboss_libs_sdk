...
cli_menu_cmd menu_config[] = {
  ...
  { "channel_change", " [channel]", "       ", config_channel_change,help_channel_change,"configure channel change to channel number [11-26]" },
...
#if defined(CLI_HAS_SLEEPY_END_DEVICE)
  { "sleepy", "", "<interval>               ", config_sleepy,        help_sleepy,        "configure sleepy end device, optional interval period <ms> with turbo polling enabled if it is provided" },
#endif
#endif
#ifdef ZB_MAC_CONFIGURABLE_TX_POWER
...
void config_init_default(void)
{
  ...
  ZB_MEMSET(&config, 0, sizeof(config));
...
#endif
#ifdef ZB_MAC_CONFIGURABLE_TX_POWER
  ZB_MEMSET(config.tx_power_per_channel, TX_POWER_NOT_INITIALIZED, sizeof(config.tx_power_per_channel));
#endif
...
/* Local functions */
static void config_get_node_desc(zb_uint16_t short_address)
{
  ...
  /* If no done, we this device ask us to read an attribute, we don't know how to fragment it, so if the attribute is too big => ZB_ZCL_STATUS_INSUFF_SPACE */
  buffer = zb_buf_get_out();
  req = zb_buf_initial_alloc(buffer, sizeof(zb_zdo_node_desc_req_t));
  req->nwk_addr = short_address;
  zb_zdo_node_desc_req(buffer, NULL);
}
...
void config_got_signal(zb_zdo_app_signal_type_t signal, zb_zdo_app_signal_hdr_t *signal_hdr)
{
  switch(signal) {
    ...
    case ZB_ZDO_SIGNAL_DEVICE_ANNCE: /* result of a new device associated */
      {
...
static zb_ret_t config_ieee_addr(int argc, char *argv[])
{
  ...
  ZB_MEMCPY(config.ieee_addr, new_ieee_addr, sizeof(zb_ieee_addr_t));
...
static zb_ret_t config_nwk_key(int argc, char *argv[])
{
  ...
  ZB_MEMCPY(config.nwk_key[new_key_number], new_nwk_key, ZB_CCM_KEY_SIZE);
...
static zb_ret_t config_extpanid(int argc, char *argv[])
{
  ...
  ZB_MEMCPY(config.extpanid, new_extpanid, sizeof(zb_ext_pan_id_t));
 ...
 * response for zb_prepare_network_for_channel_change
 */
static void channel_change_cb(zb_uint8_t param)
{
  zb_ret_t status = zb_buf_get_status(param);

  if(status != RET_OK)
  {
    menu_printf("channel_change_cb() failed: %s", wcs_get_error_str(status));
    config.channel.val.number = zb_get_current_channel();
  }
  else
  {
    ...
    if(params->error_cnt != 0)
    {
      menu_printf("channel_change_cb() had %u errors", params->error_cnt);
      config.channel.val.number = zb_get_current_channel();
    }
    else
    {
      status = zb_start_channel_change(param);
      if(status != RET_OK)
      {
        menu_printf("channel_change_cb() had no errors but start_channel_change() failed : %s", wcs_get_error_str(status));
        config.channel.val.number = zb_get_current_channel();
      }
      else
      {
        menu_printf("channel_change_cb() Ok, send Network Update Request");
        param = 0;
      }
    }
  }
 ...
 * command config
 *
 * config channel_change [channel]
 */
static zb_ret_t config_channel_change(int argc, char *argv[])
{
  zb_ret_t ret = RET_OK;
  zb_bufid_t param;
  ...
  if(!(config_is_coordinator() || (config_is_router() && config.nwk_distrib)))
   return RET_UNAUTHORIZED;
  ...
  /* Do it */
  {
    zb_channel_change_parameters_t *params;

    param = zb_buf_get_out();
    params = ZB_BUF_GET_PARAM(param, zb_channel_change_parameters_t);
    params->next_channel_change = 1U<<new_channel;

    /* Send set_configuration_req to all devices to allow channel change */
    ret = zb_prepare_network_for_channel_change(param, channel_change_cb);
    if(ret == RET_OK)
    {
      menu_printf("prepare network for channel change, next channel: %u", new_channel);
      config.channel.val.number = new_channel;
    }
  }

  return ret;
}
static zb_ret_t help_channel_change(void)
{
 ...
 *
 * config sleepy <interval>
 */
static zb_ret_t config_sleepy(int argc, char *argv[])
{
  zb_ret_t ret;
  zb_uint32_t new_interval = 5000; /* Default value: 5 sec, refer ZB_PIM_DEFAULT_LONG_POLL_INTERVAL */
  ...
  if(argc >= 1) {
    /* get <interval> */
    TOOLS_GET_ARG(ret, uint32, argv, 0, &new_interval);
    new_turbo = ZB_TRUE;
  }
...
static zb_ret_t help_sleepy(void)
{
  ...
  menu_printf("if an interval is provided, turbo polling is enabled by default");
  return RET_OK;
}
...
static zb_ret_t config_tx_power(int argc, char *argv[])
{
  ...
  if(ret != RET_OK && ret != RET_EOF) /* found or ot found */
    ...
    /* zb_get_current_channel() can be used only after device formed|joined a network */
  if(ZB_JOINED())
    current_channel = zb_get_current_channel();
  else if(config.channel.type == CHANNEL_TYPE_NUMBER)
    current_channel = config.channel.val.number;
  else if(config.channel.type == CHANNEL_TYPE_MASK)
    for(current_channel=11; current_channel<=26; current_channel++)
      if((1l<<current_channel) & config.channel.val.mask)
        break;

  if(current_channel)
  {
    /* if channel is set, validate it */
  ...
  {
    /* if channel is not set, store this power on all channels & do get/set on first channel */
    config.tx_power_index = 0;
    if(new_tx_power != TX_POWER_NOT_INITIALIZED)
      for(int ch = 11; ch < 27 ; ch++)
        config.tx_power_per_channel[ch-11] = new_tx_power;
  }
...
#if defined(ZB_TRACE_LEVEL)
  menu_printf("trace:            %u 0x%08x", zb_trace_get_level(), zb_trace_get_mask());
#endif
...
{
  zb_get_pan_channel_params_t *channel_params = zb_buf_begin(param);
  ...
  {
    menu_printf("get_pan_channel_cb() Ok: current channel %u, side pan channel %u, page %u", channel_params->current_pan_channel, channel_params->side_pan_channel, channel_params->page);
  }
...
{
  zb_set_pan_channel_params_t *channel_params = zb_buf_begin(param);
  ...
  {
    menu_printf("set_pan_channel_cb() Ok: channel %u, page %u", channel_params->current_pan_channel, channel_params->page);
  }
...
static zb_ret_t config_pan_channel(int argc, char *argv[])
{
  ...
  /* Do it */
  {
    ...
    if(get_nset_pan_ch == ZB_TRUE)
    {
      zb_get_pan_channel_params_t *channel_params;
      channel_params = zb_buf_initial_alloc(buffer, sizeof(zb_get_pan_channel_params_t));
      /* get pan channel */
      channel_params->current_pan_channel = -1;
      channel_params->side_pan_channel = -1;
    ...
    {
      zb_set_pan_channel_params_t *channel_params;
      channel_params = zb_buf_initial_alloc(buffer, sizeof(zb_set_pan_channel_params_t));
      /* set pan channel */
      channel_params->page = ZB_CHANNEL_PAGE0_2_4_GHZ;
      channel_params->current_pan_channel = new_set_channel;
 ...
 */
static zb_ret_t redirect_trace_cb(const zb_trace_handler_info_t *info, const zb_char_t *trace_msg, zb_uint16_t trace_len)
{
  ZVUNUSED(info);
  ZVUNUSED(trace_len);

  //menu_printf("ZBOSS_LOG: level: %u, mask: %u", info->level, info->mask);
  menu_printf("ZBOSS_LOG: %s", trace_msg);
 ...
 *
 * config redirect_trace [enable]
 */
static zb_ret_t config_redirect_trace(int argc, char *argv[])
{
  zb_ret_t ret;
  ...
  if(new_enable)
    zb_trace_handler_setup(redirect_trace_cb);
  else
    zb_trace_handler_setup(NULL);

  return ret;
}
static zb_ret_t help_redirect_trace(void)
{
