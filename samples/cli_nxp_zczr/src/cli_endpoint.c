...
//        zb_zcl_disc_attr_res_t *disc_attr_res = (zb_zcl_disc_attr_res_t *)zb_buf_begin(param);
        ...
        zb_uint16_t last_attr_id = 0;
        zb_uint16_t first_attr_id = 0xFFFF;
        zb_bool_t got_attributes = ZB_FALSE;
        zb_uint16_t batch_count = 0;
        ...
        while(disc_attr_info)
        {
          ...
          /* Track first and last attribute IDs */
          if (!got_attributes)
          {
            first_attr_id = disc_attr_info->attr_id;
            got_attributes = ZB_TRUE;
          }
          last_attr_id = disc_attr_info->attr_id;
          batch_count++;

          ZB_ZCL_GENERAL_GET_NEXT_DISC_ATTR_RES(param_tmp, disc_attr_info);
        }

        g_disc_attr_ctx.total_attrs += batch_count;

        /* If discovery is not complete, need to send another request */
        if (complete == 0 && got_attributes && g_disc_attr_ctx.total_attrs < g_disc_attr_ctx.max_count)
        {
          zb_uint16_t next_attr_id = last_attr_id + 1;

          /* Check for loop condition: if we got attributes and the first one
          * is LESS THAN (not equal to) what we requested, we've wrapped around */
          if (got_attributes && g_disc_attr_ctx.start_attr_id != 0 &&
              first_attr_id < g_disc_attr_ctx.start_attr_id)
          {
            menu_printf("Discovery complete - detected wrap-around (first_attr 0x%04x < requested 0x%04x)",
                      first_attr_id, g_disc_attr_ctx.start_attr_id);
            ZB_MEMSET(&g_disc_attr_ctx, 0, sizeof(g_disc_attr_ctx));
          }
          else
          {
            menu_printf("Discovery not complete, request more attributes starting from 0x%04x", next_attr_id);
            g_disc_attr_ctx.next_attr_id = next_attr_id;
            ZB_SCHEDULE_APP_CALLBACK(continue_disc_attr_wrapper, 0);
          }
        }
        else
        {
          menu_printf("Discovered %d attributes", g_disc_attr_ctx.total_attrs);
          /* Reset context */
          ZB_MEMSET(&g_disc_attr_ctx, 0, sizeof(g_disc_attr_ctx));
        }
...
static zb_ret_t endpoint_create(int argc, char *argv[])
{
  ...
  /* Params are good, insert that new endpoint in our context */
  {
    ...
    /* Allocate EP storage */
    new_ep = (zb_af_endpoint_desc_t *)ZB_MALLOC(sizeof(zb_af_endpoint_desc_t));
    if(!new_ep)
      return RET_NO_MEMORY;
    ZB_MEMSET(new_ep, 0, sizeof(zb_af_endpoint_desc_t));
    ...
    new_ep->cluster_desc_list = (zb_zcl_cluster_desc_t *)ZB_MALLOC((MAX_CLUSTERS)*sizeof(zb_zcl_cluster_desc_t));
    if(!new_ep->cluster_desc_list)
    {
      ZB_FREE(new_ep);
      return RET_NO_MEMORY;
    }
    ZB_MEMSET(new_ep->cluster_desc_list, 0, (MAX_CLUSTERS)*sizeof(zb_zcl_cluster_desc_t));
    new_ep->simple_desc       = (zb_af_simple_desc_1_1_t *)ZB_MALLOC(sizeof(STRUCT_SIMPLE_DESC_NAME));
    if(!new_ep->simple_desc)
    {
      ZB_FREE(new_ep->cluster_desc_list);
      ZB_FREE(new_ep);
      return RET_NO_MEMORY;
    }
    ZB_MEMSET(new_ep->simple_desc, 0, sizeof(STRUCT_SIMPLE_DESC_NAME));
    ...
    ZB_MEMSET(new_ep->simple_desc->app_cluster_list, 0, sizeof(new_ep->simple_desc->app_cluster_list));
    ...
    new_ep->reporting_info = (zb_zcl_reporting_info_t *)ZB_MALLOC(new_ep->rep_info_count * sizeof(zb_zcl_reporting_info_t));
    if(!new_ep->reporting_info)
    {
      ZB_FREE(new_ep->simple_desc);
      ZB_FREE(new_ep->cluster_desc_list);
      ZB_FREE(new_ep);
      return RET_NO_MEMORY;
    }
    /* Zero out the allocated reporting info to prevent garbage data */
    ZB_BZERO(new_ep->reporting_info, new_ep->rep_info_count * sizeof(zb_zcl_reporting_info_t));
