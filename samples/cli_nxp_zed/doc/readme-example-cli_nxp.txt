...
config ed_timeout [timeout]:            RO  configure network end device timeout [milliseconds]
config sleepy <interval>:               CO  configure sleepy end device, optional interval period <ms> with turbo polling enabled if it is provided
...
- cluster disc_attr [endpoint] [addr] [ep] [cluster] [start] [max]:
  ...
  cluster disc_attr [endpoint] [addr] [ep] [cluster] [start] [max] <manuf> <aps_secured> <disable_aps_ack> <delay>:
        ...
        Note: cluster disc_attr command shows an example of chained commands: Refer to ZB_ZCL_CMD_DISC_ATTRIB_RESP in cli_endpoint.c for the handling of automatic discovery requests
