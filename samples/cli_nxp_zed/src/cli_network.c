...
static zb_ret_t network_ch_update(int argc, char *argv[]);
...
cli_menu_cmd menu_network[] = {
  ...
  { "ch_update", " [disable]", "      ", network_ch_update, help_empty,     "disable channel change from unsolicited network updates enabled by default, disable [0(default)|1]" },
 ...
 * command ch_update
 *
 * network ch_update [disable]
 */
static zb_ret_t network_ch_update(int argc, char *argv[])
{
  zb_ret_t ret;
  zb_uint8_t new_disable;
  ...
  /* get [enable] */
  TOOLS_GET_ARG(ret, uint8, argv, 0, &new_disable);
  if(new_disable >= 2)
      return RET_INVALID_PARAMETER_1;

  /* Do it */
  {
    zb_zdo_disable_network_mgmt_channel_update(new_disable);
  }
