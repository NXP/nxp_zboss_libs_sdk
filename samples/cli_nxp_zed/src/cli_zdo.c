...
static void get_node_desc_cb(zb_uint8_t param)
{
  ...
  else
  {
    ...
    menu_printf("get_node_desc_cb(0x%04x): Capability info %02x, Manufacturer: 0x%04x, max buf %hu, max transfer in %hu out %hu",
      resp->hdr.nwk_addr,
...
static zb_ret_t zdo_get_match_desc(int argc, char *argv[])
{
  ...
  cluster_list = ZB_MALLOC(nb_clusters*sizeof(zb_uint16_t));
  if(!cluster_list)
...
static zb_ret_t zdo_raw_cmd(int argc, char *argv[])
{
  ...
  /* Do it */
  {
    ...
    if(dest_payload_len > 0)
    {
      ...
      ZB_MEMCPY(ptr_buf, dest_payload_data, dest_payload_len);
    }
...
static zb_ret_t zdo_aps_cmd(int argc, char *argv[])
{
  ...
  if(argc == 3)
  {
    ...
    TOOLS_GET_ARG_HEX_ARRAY_LEN(ret, argv, 2, &dest_payload_len);
    /* Index 0 for TSN */
    dest_payload_data = ZB_MALLOC(sizeof(zb_uint8_t)*dest_payload_len + 1);
    if(!dest_payload_data)
      return RET_NO_MEMORY;
    /* get <payload data> */
