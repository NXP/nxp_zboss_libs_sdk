...
/* automatically send on-off until a button is pressed */
zb_bool_t work_via_alarm = ZB_TRUE;
...
static void bind_level_control_cluster(zb_uint8_t param)
{
  ...
  ZB_IEEE_ADDR_COPY(req->src_address, &g_ed_addr);
  ZB_IEEE_ADDR_COPY(req->dst_address.addr_long, g_device_ctx.bulb_params.ieee_addr);

  req->src_endp = LIGHT_CONTROL_ENDPOINT;
  req->cluster_id = ZB_ZCL_CLUSTER_ID_LEVEL_CONTROL;
  req->dst_addr_mode = ZB_BIND_DST_ADDR_MODE_64_BIT_EXTENDED;
  req->dst_endp = g_device_ctx.bulb_params.endpoint;
  req->req_dst_addr = zb_get_short_address();

  zb_zdo_bind_req(param, bulb_binding_completed);
}
...
static void bind_on_off_cluster(zb_uint8_t param)
{
  ...
  zb_zdo_bind_req(param, bind_level_control_cluster);
}
...
{
  TRACE_MSG(TRACE_APP1, ">> light_control_leave_nwk param %hd", (FMT__H, param));
  ...
  TRACE_MSG(TRACE_APP1, "<< light_control_leave_nwk", (FMT__0));
}


void light_control_retry_join(zb_uint8_t param)
{
  TRACE_MSG(TRACE_APP1, "light_control_retry_join %hd", (FMT__H, param));
...
{
  TRACE_MSG(TRACE_APP1, ">> light_control_leave_and_join param %hd", (FMT__H, param));
  if (ZB_JOINED())
  {
  ...
  TRACE_MSG(TRACE_APP1, "<< light_control_leave_and_join", (FMT__0));
}
...
void light_control_button_handler(zb_uint8_t button_no)
{
  ...
  short_expired = (ZB_TIME_SUBTRACT(current_time, g_device_ctx.button.timestamp) > LIGHT_CONTROL_BUTTON_THRESHOLD) ?
    (ZB_TRUE) : (ZB_FALSE);
...
void light_control_button_pressed(zb_uint8_t button_no)
{
  ...
  case LIGHT_CONTROL_BUTTON_STATE_PRESSED:
    /* It is unlike but possible situation when release for the previous long
     * press wasn't detected inside light_control_button_handler(), but user
     * already did release the button and pressed again. Just ignore it. */
    TRACE_MSG(TRACE_APP1, "Button press event while previous release wasn't detected", (FMT__0));
    break;
  default:
    break;
  }
